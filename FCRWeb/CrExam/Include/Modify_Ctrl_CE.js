/****************************************************************************
  ...
  @file Modify_Ctrl_CE.js

  @brief Modify_Ctrl_CEのクライアントスクリプト
  ...
  @date  06/03/02  HSK山本     V2.0       DicomStorage出力機能 
...
//2005/04/23 027 H.SAITO
var TARGET_MODIFYMAIN_VIEW            = "MODIFYMAIN_VIEW";
var TARGET_MODIFYMAIN_PROC            = "MODIFYMAIN_PROC";
var TARGET_ADDMENU_VIEW               = "ADDMENU_VIEW";
var TARGET_ADDMENU_PROC               = "ADDMENU_PROC";
var TARGET_CHANGEMENU_VIEW            = "CHANGEMENU_VIEW";
var TARGET_CHANGEMENU_PROC            = "CHANGEMENU_PROC";
var TARGET_DELETEMENU_VIEW            = "DELETEMENU_VIEW";
var TARGET_DELETEMENU_PROC            = "DELETEMENU_PROC";
var TARGET_SORTMENU_VIEW              = "SORTMENU_VIEW";
var TARGET_SORTMENU_PROC              = "SORTMENU_PROC";
var TARGET_CHANGEIMG_VIEW             = "CHANGEIMG_VIEW";
var TARGET_CHANGEIMG_PROC             = "CHANGEIMG_PROC";
var TARGET_OUTPUT_VIEW                = "OUTPUT_VIEW";
var TARGET_OUTPUT_PROC                = "OUTPUT_PROC";
var TARGET_OUTPUTDETAIL_VIEW          = "OUTPUTDETAIL_VIEW";
var TARGET_OUTPUTDETAIL_PROC          = "OUTPUTDETAIL_PROC";
var TARGET_PATIENTMAIN_VIEW           = "EDITPATIENTMAIN_VIEW";
var TARGET_PATIENTMAIN_PROC           = "EDITPATIENTMAIN_PROC";
var TARGET_EDITPATIENTID_VIEW         = "EDITPATIENTID_VIEW";
var TARGET_EDITPATIENTID_PROC         = "EDITPATIENTID_PROC";
var TARGET_EDITPATIENTDETAIL_VIEW     = "EDITPATIENTDETAIL_VIEW";
var TARGET_EDITPATIENTDETAIL_PROC     = "EDITPATIENTDETAIL_PROC";
var TARGET_CHANGEPATIENTID_VIEW       = "CHANGEPATIENTID_VIEW";
var TARGET_CHANGEPATIENTID_PROC       = "CHANGEPATIENTID_PROC";
var TARGET_CHANGEPATIENTDETAIL_VIEW   = "CHANGEPATIENTDETAIL_VIEW";
var TARGET_CHANGEPATIENTDETAIL_PROC   = "CHANGEPATIENTDETAIL_PROC";
//2005/12/14 PVCS#1668 H.SAITO -ST-
var TARGET_MODIFYMAIN_END             = "MODIFYMAIN_END";
var TARGET_ADDMENU_END                = "ADDMENU_END";
var TARGET_CHANGEMENU_END             = "CHANGEMENU_END";
var TARGET_DELETEMENU_END             = "DELETEMENU_END";
var TARGET_SORTMENU_END               = "SORTMENU_END";
var TARGET_CHANGEIMG_END              = "CHANGEIMG_END";
var TARGET_OUTPUT_END                 = "OUTPUT_END";
var TARGET_OUTPUTDETAIL_END           = "OUTPUTDETAIL_END";
var TARGET_PATIENTMAIN_END            = "EDITPATIENTMAIN_END";
var TARGET_EDITPATIENTID_END          = "EDITPATIENTID_END";
var TARGET_EDITPATIENTDETAIL_END      = "EDITPATIENTDETAIL_END";
var TARGET_CHANGEPATIENTID_END        = "CHANGEPATIENTID_END";
var TARGET_CHANGEPATIENTDETAIL_END    = "CHANGEPATIENTDETAIL_END";
//2005/12/14 PVCS#1668 H.SAITO -ED-

//2005/04/23 002 H.SAITO
var VIEW_MODE_FRAMEVIEW           = "FRAME_VIEW";
...
var FILE_NAME = "Modify_Ctrl_CE.js";  //ファイル名
...
//2005/04/23 005 H.SAITO
//ModifyCtrlCEかどうか
var isModifyCtrlCE = true;
...
function Fn_InitPage(){
	try{
		...
		//ページ情報追加
		loader.AddLoadPage(VIEW_MODE_FRAMEVIEW,"./ModifyMain.aspx?OpenMode=" + OpenMode);
...
// CHANGE ============================
    ...
    Public_ErrorDisplay(FATAL_ERROR, MESSAGE_ID, FILE_NAME, SPOT_CODE+3);
		...
		if(viewSrc){
			ReplaceLoad(moveViewId,procSrc,viewSrc,FC_MOVING_MODE_INIT,null,movedParam);
		}
...
function ViewFinished(viewId, viewInfo){
	try{
		...
		var procSrc = null;
		var viewSrc = null;
		var initFunction = "Public_Init";
		var initFunctionParam = null;
		...
		switch(viewId){
			...
			if(viewInfo.commandMode == COMMAND_MODE_CANCEL){
				...
				if(viewId == VIEW_MODE_OUTPUT){
					if(viewInfo.commandMode == COMMAND_MODE_DETAIL){
        				...
        				moveViewId = VIEW_MODE_OUTPUTDETAIL;
			...
			if(viewInfo.commandMode == COMMAND_MODE_CANCEL){
                ...
                moveViewId = VIEW_MODE_CHANGEPATIENTDETAIL;
                movedParam = { "focusMode" : "TextBox"};
			}
			break;
			// 患者詳細変更画面
		...
		if(executeFunction){//実行関数が設定されていたら 
			executeFunction();
		}else if(viewSrc){//Viewだけでも指定されていれば
			//replaceページロード処理を行う
			ReplaceLoad(moveViewId,procSrc,viewSrc,movingMode,movingParam,movedParam);
		}
		// 画面フレームIDを設定
		ViewId = viewId;
...
//     
//***************************************************************************
function ReplaceLoad(viewId,procSrc,viewSrc,movingMode,movingParam,movedParam)
{
	try{
	    // 画面の取得中～切り替えまでの間は元の画面で操作可能となるため、本タイミングで画面を非表示
	    ClearView();

		//ページローダ生成
		var loader = new PageLoader();
		
		if(procSrc){//Procページ情報追加
			loader.AddLoadPage("FRAME_PROC",procSrc);
		}
		if(viewSrc){//Viewページ情報追加
			loader.AddLoadPage("FRAME_VIEW",viewSrc);
			//画面遷移処理をコールバック設定

            var moveViewInfo = new MoveViewInfo("FRAME_VIEW",viewId);
            moveViewInfo.SetMovingMode(movingMode);//遷移モード
            ...
            loader.SetAllPageLoadedNotification(MoveView,moveViewInfo);
		}
		
		
		//replaceロード開始

		loader.Start(LOADMODE_REPLACE);
...
// 2009/12/25 ADD TYS松岡 CQ#61対応 End   ------------------------------------
// 2010/04/19 ADD TYS園木 責任者マルチバイト対応 Start ------------------------------------
		ResponsiblePersonIdoGraphic    = ""; //責任者(個人)
// 2010/04/19 ADD TYS園木 責任者マルチバイト対応 End   ------------------------------------
...
// 2009/12/25 ADD TYS松岡 CQ#61対応 End   ------------------------------------
// 2010/04/19 ADD TYS園木 責任者マルチバイト対応 Start ------------------------------------
		EditResponsiblePersonIdoGraphic    = ""; //責任者(個人)
// 2010/04/19 ADD TYS園木 責任者マルチバイト対応 End   ------------------------------------
		GetDate              = ""; //取得時間 
...
//ADD EN===========================
    ...
    // 2005/11/30 PVCS#1560 H.SAITO -ED-
    //2005/04/23 003 H.SAITO
    FRAME_VIEW.location.replace("about:blank");
    FRAME_PROC.location.replace("about:blank");
	}catch(e){
