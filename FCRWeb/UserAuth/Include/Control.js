...
function keydown() {
    if (ie) {
        // 2005/08/06 010 H.SAITO #1110 パスワード入力画面でEnterボタン押下でバグる不具合修正
        // 2005/10/19 神立 V1.4 [戻る]ボタンでEnterを押した時もログインしてしまう不具合を修正
        //Enter
        if (event.keyCode == 13) {
            event.keyCode = 0;
            window.event.returnValue = false;
            if(event.srcElement.tagName == "INPUT" && parent.ProcFlag == 0 
            && event.srcElement.id != "btnBack"){ // <<< 061019神立 
              parent.ProcFlag = 1;
              userConfirm();
            }
        }
